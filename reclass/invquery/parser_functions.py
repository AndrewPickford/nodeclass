...
Tags = enum.Enum('Tags', ['VALUE_QUERY', 'IF_QUERY', 'LIST_IF_QUERY', 'OPTION', 'IF',
    'OP_TEST', 'OP_LOGICAL', 'STRING', 'INT', 'FLOAT', 'EXPORT', 'PARAMETER'])
...
def inventory_query_parser():
    ...
    operator_test = (op_eq | op_neq).setParseAction(tag(Tags.OP_TEST.value))
    operator_logical = (op_and | op_or).setParseAction(tag(Tags.OP_LOGICAL.value))
    begin_if = pp.CaselessLiteral(_IF).setParseAction(tag(Tags.IF.value))

    quoted_string = pp.QuotedString('\'"').setParseAction(tag(Tags.STRING.value))
    string = pp.Word(pp.printables).setParseAction(tag(Tags.STRING.value))

    export = pp.Group(pp.CaselessLiteral(_EXPORT) + string).setParseAction(tag(Tags.EXPORT.value))
    parameter = pp.Group((pp.CaselessLiteral(_PARAMETER[0]) | pp.CaselessLiteral(_PARAMETER[1])) + string).setParseAction(tag(Tags.PARAMETER.value))
    ...
    expritem = integer | real | quoted_string | export | parameter | string
